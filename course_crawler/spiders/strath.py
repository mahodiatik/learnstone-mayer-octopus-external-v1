...
class StrathSpider(scrapy.Spider):
    ...
    async def parse_course(self, response: HtmlResponse):
        ...
        if qualifications.find("/") != -1:
            for qualification in qualifications.split("/"):
                tuitions = self._get_tuitions(soup, qualification,len(qualifications.split("/"))>1)
                yield {
                    "title": title,
            ...
            tuitions = self._get_tuitions(soup, qualifications,False)
            yield {
                "title": title,
                "link": response.meta["course_link"],
                "study_level": study_level,
    ...
    def _get_duration(self,soup: BeautifulSoup) -> list[dict]: 
        try:
            durations = {}
    ...
    def _get_tuitions(self,soup: BeautifulSoup, parsed_qualification: str, multiple: bool):
        try:
            tuitions=[]
            try:
                duration_text=soup.select_one("span.fa.fa-pencil-square-o").next_sibling.get_text().strip()
