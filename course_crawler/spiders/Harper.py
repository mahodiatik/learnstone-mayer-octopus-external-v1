...
class HarperSpider(scrapy.Spider):
    ...
    def _get_description(self, course: Tag):
        try:
            description=course.select_one("#overview div div div div.flex-width-six div").text.replace("\n","").strip()
        except AttributeError:
            description = ""
        return description
    ...
    def _get_tuitions(self, soup: BeautifulSoup, criteria: str):
        ...
        try:
            ...
            try:
                selector=soup.select("#key-course-info div div div div div h3")
                for i in selector:
                    if(i.text=="Duration"):
                        text=i.find_next_sibling()
                        part_time=text.find("br").next_sibling.text
                        duration=re.search(r'^([^()]+)', part_time).group(1).strip()
                        if(duration.find("part-time")!=1):
                            duration=duration.replace("part-time","").strip()
                        study_mode="part-time"
                        student_category="All"
                        fee="£64 per credit" # part-time fee is given in https://cdn.harper-adams.ac.uk/document/ki/finance/Fees-and-Charges-202425.pdf
                        tuitions.append(
                            {
